...
class CocoDetectionDataset(CocoDataset):
    ...
    def _load_anns(self, index):
        ...
        return np.array(anns, dtype=np.float)
...
class Yolov5MosaicDataset(Yolov5Dataset):
    def __init__(self, deepvac_config):
        self.conf.augment.border = [-deepvac_config.img_size // 2] * 2
...
if __name__ == '__main__':
    ...
    loader = DataLoader(dataset, batch_size=1, shuffle=True, collate_fn=Yolov5Dataset.collate_fn)
    for img, t in loader:
        img = img.float()
        print("img: ", img.sum(dim=1), img.mean(dim=1))
        img /= 255.
        print("image: ", img.shape, img.sum(), img.mean(), img.max(), img.min())
        print(t)
